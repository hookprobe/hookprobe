"""
OpenFlow SDN Controller for HookProbe
...
OFP_CONSTANTS = {
    'VERSION': OFP_VERSION,
    'HEADER_SIZE': OFP_HEADER_SIZE,
    'TCP_PORT': OFP_TCP_PORT,
    'LEGACY_PORT': OFP_LEGACY_PORT
}
...
# VLAN ranges for network segmentation
class VLANRange(IntEnum):
    MANAGEMENT = 10
    TRUSTED = 100
    GUEST = 200
    IOT = 300
...
class OpenFlowController:
    ...
    async def _handle_packet_in(self, body: bytes, writer: asyncio.StreamWriter):
        ...
        vlan_id = self.mac_to_vlan.get(eth_src.lower(), VLANRange.GUEST)
    ...
    async def _quarantine_packet(
        ...
        actions = [
            ...
            FlowAction(OFPActionType.SET_FIELD, vlan_vid=VLANRange.QUARANTINE),
    ...
    async def quarantine_mac(self, mac: str, reason: str = "suspicious"):
        ...
        self.mac_to_vlan[mac] = VLANRange.QUARANTINE

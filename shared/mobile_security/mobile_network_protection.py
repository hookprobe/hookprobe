...
@dataclass
class MobileProtectionConfig:
    """Configuration for mobile network protection"""
    data_dir: str = "/opt/hookprobe/data"
    vpn_interface: str = "wg0"
    auto_vpn: bool = True
    scan_interval: int = 60
    trust_timeout: int = 86400  # 24 hours
    enable_captive_detection: bool = True
...
class MobileNetworkProtection:
    ...
    def _check_dns_security(self, profile: NetworkProfile):
        ...
        if dns_hijack_detected:
            ...
            # Record threat
            self.threats.append(NetworkThreat(
                threat_type=ThreatType.DNS_HIJACK,
                severity="critical",
                description="DNS responses are being manipulated",
            ...
            if known.gateway_mac and profile.gateway_mac:
                if known.gateway_mac != profile.gateway_mac:
                    ...
                    self.threats.append(NetworkThreat(
                        ...
                        severity="critical",
                        description="Gateway MAC address changed - potential MITM",
                        evidence={'expected_mac': known.gateway_mac, 'current_mac': profile.gateway_mac},
                        detected_at=datetime.now()
                    ))
                    return
    ...
    def _check_ssl_interception(self, profile: NetworkProfile):
        ...
        if ssl_issues:
            ...
            # Record threat
            self.threats.append(NetworkThreat(
                threat_type=ThreatType.SSL_STRIPPING,
                severity="critical",
                description="SSL/TLS interception detected",
                evidence={'ssl_issues': ssl_issues},
                detected_at=datetime.now()
            ))
        else:
    ...
    def _check_dhcp_security(self, profile: NetworkProfile):
        ...
        if success and output:
            ...
            if len(dhcp_servers) > 1:
                ...
                # Record threat
                self.threats.append(NetworkThreat(
                    threat_type=ThreatType.ROGUE_DHCP,
                    severity="high",
    ...
    def _check_arp_security(self, profile: NetworkProfile):
        ...
        if duplicates:
            ...
            self.threats.append(NetworkThreat(
                ...
                severity="critical",
                description="ARP spoofing detected - IPs with multiple MACs",
                evidence={'duplicate_ips': duplicates},
                detected_at=datetime.now()
            ))
        else:

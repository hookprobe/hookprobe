#!/bin/bash
# ============================================================
# HookProbe Shared WiFi DFS Capability Detection
# ============================================================
#
...
#   - Qualcomm/Atheros (ath10k/ath11k/ath12k): QCA6174/QCN9274/QCN6224 - Full DFS
...
# Includes both vendor IDs: 168c (legacy Atheros) and 17cb (Qualcomm)
declare -A QUALCOMM_DFS_CAPS=(
    # WiFi 7 (ath12k) - QCN9274 and QCN6224 added
    ["1107"]="full:WCN7850:wifi7"
    ["1109"]="full:QCN9274:wifi7"      # Qualcomm WiFi 7 AP chipset
    ["110a"]="full:QCN6224:wifi7"      # Qualcomm WiFi 7 client chipset
    # WiFi 6E (ath11k)
...
# Additional Qualcomm PCI IDs for QCN9274/QCN6224 variants
declare -A QUALCOMM_NEW_DFS_CAPS=(
    # QCN9274 variants (WiFi 7 AP)
    ["1109"]="full:QCN9274:wifi7"
    ["9274"]="full:QCN9274:wifi7"
    # QCN6224 variants (WiFi 7 client)
    ["110a"]="full:QCN6224:wifi7"
    ["6224"]="full:QCN6224:wifi7"
    # WCN7850 (WiFi 7 client - common on laptops)
    ["1107"]="full:WCN7850:wifi7"
    # QCN9074 (WiFi 6E enterprise AP)
    ["1101"]="full:QCN9074:wifi6e"
)
...
get_chipset_dfs_capability() {
    ...
    case "$vendor" in
        ...
        14c3)  # MediaTek
            ...
            ;;
        168c|17cb)  # Qualcomm/Atheros (both legacy and new vendor IDs)
            if [ -n "${QUALCOMM_DFS_CAPS[$device]}" ]; then
                echo "${QUALCOMM_DFS_CAPS[$device]}"
                return 0
            fi
            # Check new Qualcomm chipsets database
            if [ -n "${QUALCOMM_NEW_DFS_CAPS[$device]}" ]; then
                echo "${QUALCOMM_NEW_DFS_CAPS[$device]}"
                return 0
            fi
            ;;
        10ec)  # Realtek
...
Supported Vendors:
  ...
  - Qualcomm (ath10k/ath11k/ath12k): QCA6174/QCN9274/QCN6224

...
# HookProbe Website Integration
# When hookprobe-com website is running, we integrate with its proxy
HOOKPROBE_COM_DIR="/home/ubuntu/hookprobe-com"
HOOKPROBE_COM_RUNNING=false
HOOKPROBE_COM_NETWORK="hookprobe-public"
...
detect_hookprobe_com() {
    log_section "Detecting HookProbe Website Integration"

    # Check if hookprobe-com proxy is running (as non-root user)
    # We need to check the regular user's containers
    local proxy_running=false

    # Find the user who owns hookprobe-com directory
    if [ -d "$HOOKPROBE_COM_DIR" ]; then
        HOOKPROBE_COM_OWNER=$(stat -c '%U' "$HOOKPROBE_COM_DIR")

        # Check if that user has hookprobe-proxy container running
        if sudo -u "$HOOKPROBE_COM_OWNER" podman ps --format "{{.Names}}" 2>/dev/null | grep -q "^hookprobe-proxy$"; then
            proxy_running=true
        fi
    fi

    if $proxy_running; then
        HOOKPROBE_COM_RUNNING=true
        log_success "HookProbe website detected (hookprobe-proxy running)"
        log_info "MSSP will integrate with existing proxy on ports 80/443"
        log_info "MSSP Django will be connected to $HOOKPROBE_COM_NETWORK network"
        log_info "Owner user: $HOOKPROBE_COM_OWNER"

        # Check if the network exists
        if sudo -u "$HOOKPROBE_COM_OWNER" podman network exists "hookprobe-com_hookprobe-public" 2>/dev/null; then
            log_success "Network hookprobe-com_hookprobe-public exists"
        else
            log_warning "Network hookprobe-com_hookprobe-public not found"
        fi
    else
        log_info "HookProbe website not running"
        log_info "MSSP will deploy its own nginx proxy on ports 80/443"
    fi
}
...
deploy_with_hookprobe_com() {
    log_section "Deploying MSSP with HookProbe-COM Integration"

    local owner_user="$HOOKPROBE_COM_OWNER"
    local compose_file="$HOOKPROBE_COM_DIR/podman-compose.yml"

    # Verify compose file exists and has MSSP profile
    if [ ! -f "$compose_file" ]; then
        log_error "podman-compose.yml not found at $compose_file"
        log_error "Please ensure hookprobe-com is properly set up"
        exit 1
    fi

    if ! grep -q "profile.*mssp\|profiles.*mssp" "$compose_file"; then
        log_error "MSSP profile not found in $compose_file"
        log_error "Please update hookprobe-com to the latest version with MSSP support"
        exit 1
    fi

    # Generate secrets if they don't exist
    log_info "Generating MSSP secrets..."
    local mssp_secret_key=$(openssl rand -base64 50 | tr -dc 'a-zA-Z0-9' | head -c 64)
    local mssp_postgres_password=$(openssl rand -base64 32 | tr -dc 'a-zA-Z0-9' | head -c 32)

    # Create .env file for MSSP in hookprobe-com directory
    local env_file="$HOOKPROBE_COM_DIR/.env.mssp"
    log_info "Creating MSSP environment file: $env_file"

    cat > "$env_file" << EOF
# MSSP Configuration - Generated by setup.sh
# $(date)

# Django settings
MSSP_DJANGO_SECRET_KEY=$mssp_secret_key

# PostgreSQL settings
MSSP_POSTGRES_DB=hookprobe
MSSP_POSTGRES_USER=hookprobe
MSSP_POSTGRES_PASSWORD=$mssp_postgres_password

# Path to hookprobe repo (for building MSSP container)
HOOKPROBE_REPO_PATH=$HOOKPROBE_ROOT
HOOKPROBE_COM_PATH=$HOOKPROBE_COM_DIR
EOF

    chown "$owner_user:$owner_user" "$env_file"
    chmod 600 "$env_file"

    # Build and deploy MSSP containers using podman-compose
    log_info "Building MSSP Django container..."
    cd "$HOOKPROBE_COM_DIR"

    # Source environment file and run as the owner user
    sudo -u "$owner_user" bash -c "
        set -a
        source $env_file
        set +a
        cd $HOOKPROBE_COM_DIR
        podman-compose --profile mssp build mssp-django
    "

    log_info "Starting MSSP containers..."
    sudo -u "$owner_user" bash -c "
        set -a
        source $env_file
        set +a
    ...
    while [ $attempt -le $max_attempts ]; do
        if sudo -u "$owner_user" podman exec mssp-postgres pg_isready -U hookprobe 2>/dev/null; then
    ...
    sudo -u "$owner_user" podman exec mssp-django python manage.py migrate

    # Collect static files
    log_info "Collecting static files..."
    sudo -u "$owner_user" podman exec mssp-django python manage.py collectstatic --noinput 2>/dev/null || true

    # Create superuser
    log_info "Creating admin superuser..."
    sudo -u "$owner_user" podman exec mssp-django python manage.py createsuperuser --noinput \
        --username admin --email admin@hookprobe.com 2>/dev/null || log_info "Admin user may already exist"

    # Set admin password
    local admin_password=$(openssl rand -base64 16 | tr -dc 'a-zA-Z0-9' | head -c 16)
    sudo -u "$owner_user" podman exec mssp-django python manage.py shell -c "
...
"
    ...
    # Reload nginx to ensure MSSP routing is active
    log_info "Reloading nginx proxy..."
    sudo -u "$owner_user" podman exec hookprobe-proxy nginx -t && \
    sudo -u "$owner_user" podman exec hookprobe-proxy nginx -s reload

    # Health check
    log_info "Running health check..."
    sleep 5
    if sudo -u "$owner_user" podman exec mssp-django python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8000/health/')" 2>/dev/null; then
        log_success "MSSP Django is healthy"
    else
        log_warning "MSSP Django health check failed - may still be starting"
    fi

    # Save admin credentials
    local creds_file="/etc/hookprobe/mssp-admin-credentials.txt"
    mkdir -p /etc/hookprobe
    cat > "$creds_file" << EOF
# MSSP Admin Credentials
# Generated: $(date)
#
# URL: https://mssp.hookprobe.com/login/
# Username: admin
...
EOF
    ...
    log_section "MSSP Installation Complete (HookProbe-COM Integration)"
    echo ""
    log_success "MSSP has been deployed successfully!"
    echo ""
    echo -e "  ${CYAN}Access MSSP Dashboard:${NC}"
    echo -e "    URL:      ${GREEN}https://mssp.hookprobe.com/login/${NC}"
    ...
    sudo -u "$owner_user" podman ps --filter "name=mssp-" --format "table {{.Names}}\t{{.Status}}"
    echo ""
    echo -e "  ${YELLOW}Note: Change the admin password after first login!${NC}"
    echo ""
}
...
deploy_pod_001_dmz() {
    log_section "Deploying POD-001: DMZ (Django)"
    ...
    # =============================================================
    # NGINX DEPLOYMENT - Conditional based on hookprobe-com
    # =============================================================

    if $HOOKPROBE_COM_RUNNING; then
        # Integrate with existing hookprobe-com proxy
        integrate_with_hookprobe_com "$django_container"
    else
        # Deploy standalone MSSP nginx
        deploy_mssp_nginx "$network" "$external_network"
    fi
}

integrate_with_hookprobe_com() {
    local django_container="$1"

    log_info "Integrating MSSP Django with hookprobe-com proxy..."

    # Find the user who owns hookprobe-com
    local owner_user
    owner_user=$(stat -c '%U' "$HOOKPROBE_COM_DIR")

    # Get the MSSP Django container's IP on mssp-pod-001-dmz network
    local mssp_django_ip
    mssp_django_ip=$(podman inspect "$django_container" --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' 2>/dev/null | head -1)

    if [ -z "$mssp_django_ip" ]; then
        mssp_django_ip="172.20.1.10"
    fi

    log_info "MSSP Django IP: $mssp_django_ip"

    # Update hookprobe-com nginx config to enable MSSP routing
    update_hookprobe_com_nginx "$owner_user" "$mssp_django_ip"

    # Expose MSSP Django port on host for cross-network access
    # The hookprobe-com proxy will reach MSSP via host port
    log_info "Exposing MSSP Django on host port 8001..."
    ...
    # Reconnect to internal networks
    podman network connect mssp-pod-003-db "$django_container"
    podman network connect mssp-pod-004-cache "$django_container"

    log_success "MSSP Django exposed on localhost:8001 for hookprobe-com proxy"
    log_info "Skipping MSSP nginx deployment (using hookprobe-com proxy)"
}

update_hookprobe_com_nginx() {
    local owner_user="$1"
    local mssp_ip="$2"

    local nginx_conf="$HOOKPROBE_COM_DIR/containers/proxy/nginx.conf"

    if [ ! -f "$nginx_conf" ]; then
        log_warning "hookprobe-com nginx.conf not found at $nginx_conf"
        return 1
    fi

    log_info "Updating hookprobe-com nginx config for MSSP routing..."

    # Backup original
    cp "$nginx_conf" "${nginx_conf}.backup.$(date +%Y%m%d%H%M%S)"

    # Create updated nginx config with MSSP upstream enabled
    cat > "$nginx_conf" << 'NGINX_EOF'
# HookProbe Reverse Proxy - Subdomain Routing
# Auto-updated by MSSP setup.sh for MSSP integration
#
# Routes:
#   hookprobe.com      → website container (Django)
#   mssp.hookprobe.com → mssp-django container (MSSP Django)
...
# MSSP upstream - connects to MSSP Django via host port
upstream mssp {
...
# Main website: hookprobe.com
server {
    listen 80 default_server;
    server_name hookprobe.com www.hookprobe.com localhost _;
...
server {
    listen 80;
    server_name mssp.hookprobe.com mssp.localhost;
    ...
    # WebSocket for Cortex and real-time updates
    location /ws/ {
        proxy_pass http://mssp;
        proxy_http_version 1.1;
    ...
    # Static files (served from MSSP Django)
    location /static/ {
        proxy_pass http://mssp;
        proxy_cache_valid 200 30d;
        expires 30d;
...
# =============================================================
# HTTPS Server (Production - Uncomment when SSL is ready)
# =============================================================
# server {
#     listen 443 ssl http2;
#     server_name hookprobe.com www.hookprobe.com;
#     ssl_certificate /etc/nginx/ssl/hookprobe.com.crt;
#     ssl_certificate_key /etc/nginx/ssl/hookprobe.com.key;
#     ssl_protocols TLSv1.2 TLSv1.3;
#     # ... (similar to HTTP config above)
# }
#
# server {
#     listen 443 ssl http2;
#     server_name mssp.hookprobe.com;
#     ssl_certificate /etc/nginx/ssl/mssp.hookprobe.com.crt;
#     ssl_certificate_key /etc/nginx/ssl/mssp.hookprobe.com.key;
#     ssl_protocols TLSv1.2 TLSv1.3;
#     # ... (similar to HTTP config above)
# }
NGINX_EOF
    ...
    # Fix ownership
    chown "$owner_user:$owner_user" "$nginx_conf"

    # Reload nginx in hookprobe-proxy container
    log_info "Reloading hookprobe-proxy nginx..."
    if sudo -u "$owner_user" podman exec hookprobe-proxy nginx -t 2>/dev/null; then
        sudo -u "$owner_user" podman exec hookprobe-proxy nginx -s reload
        log_success "hookprobe-com nginx reloaded with MSSP routing"
    else
...
deploy_mssp_nginx() {
    ...
    log_info "Deploying standalone MSSP nginx proxy..."
...
deploy_pod_006_security() {
    ...
    podman run -d \
        ...
        --network "$network" \
        --ip "$qsecbit_ip" \
        --health-cmd="python -c 'import urllib.request; urllib.request.urlopen(\"http://localhost:8888/health\")'" \
        --health-interval=30s \
        --health-retries=3 \
...
health_check() {
    ...
    # Check core containers (always deployed)
    ...
    # Check nginx only if not using hookprobe-com proxy
    if ! $HOOKPROBE_COM_RUNNING; then
        if podman ps --format "{{.Names}}" | grep -q "^mssp-nginx$"; then
            log_success "mssp-nginx: Running"
        else
            log_error "mssp-nginx: Not running"
            ((errors++))
        fi
    else
        # Check hookprobe-proxy instead
        local owner_user
        owner_user=$(stat -c '%U' "$HOOKPROBE_COM_DIR" 2>/dev/null || echo "")
        if [ -n "$owner_user" ] && sudo -u "$owner_user" podman ps --format "{{.Names}}" 2>/dev/null | grep -q "^hookprobe-proxy$"; then
            log_success "hookprobe-proxy: Running (integrated mode)"
        else
            log_warning "hookprobe-proxy: Not running (MSSP may not be accessible)"
        fi
    fi
...
show_completion() {
    ...
    if $HOOKPROBE_COM_RUNNING; then
        echo -e "${CYAN}  Mode: Integrated with HookProbe Website${NC}"
        echo "  ─────────────────────────────────────────────"
        echo ""
        echo "  Access URLs (via hookprobe-com proxy):"
        echo "  ─────────────────────────────────────────────"
        echo "  HookProbe Website: http://localhost/ or http://hookprobe.com/"
        echo "  MSSP Dashboard:    http://mssp.localhost/ or http://mssp.hookprobe.com/"
        echo ""
        echo "  To test locally, add to /etc/hosts:"
        echo "    127.0.0.1  mssp.localhost"
        echo ""
    else
        echo -e "${CYAN}  Mode: Standalone MSSP${NC}"
        echo "  ─────────────────────────────────────────────"
        echo ""
        echo "  Access URLs:"
        echo "  ─────────────────────────────────────────────"
    ...
    echo "  Grafana:           http://localhost:3000/ (internal)"
    echo "  n8n Automation:    http://localhost:5678/ (internal)"
    echo "  Logto Admin:       http://localhost:3002/ (internal)"
    echo ""
    ...
    echo "  MSSP Containers:   sudo podman ps -a --filter 'name=mssp-'"
    if $HOOKPROBE_COM_RUNNING; then
        echo "  Website Containers: podman ps -a --filter 'name=hookprobe-'"
    fi
    echo ""

...
dhcp-range=vlan${VLAN_LAN},${DHCP_START_LAN},${DHCP_END_LAN},255.255.255.0,24h
...
dhcp-range=vlan${VLAN_MGMT},${DHCP_START_MGMT},${DHCP_END_MGMT},255.255.255.0,24h
...
# DNS Settings
# ============================================================
# Use dnsXai container for DNS (if available), fallback to Cloudflare
# server=172.20.200.21#5353
server=1.1.1.1
server=8.8.8.8
...
EOF
    ...
    elif systemctl is-active --quiet fortress-dnsmasq; then
        systemctl restart fortress-dnsmasq
        log_info "fortress-dnsmasq restarted"
    fi
}
...
table inet fortress_vlan {
    ...
    chain forward {
        ...
        iifname "vlan100" oifname { "eth0", "wwan0", "wlan0" } accept
        ...
        iifname "vlan200" oifname { "eth0", "wwan0", "wlan0" } accept
        ...
        ip saddr 172.20.200.0/24 oifname { "eth0", "wwan0", "wlan0" } accept
    ...
    chain postrouting {
        ...
        iifname "vlan100" oifname { "eth0", "wwan0", "wlan0" } masquerade

        # Masquerade Management traffic to internet
        iifname "vlan200" oifname { "eth0", "wwan0", "wlan0" } masquerade

        # Masquerade Container traffic to internet
        ip saddr 172.20.200.0/24 oifname { "eth0", "wwan0", "wlan0" } masquerade
    }
}
EOF

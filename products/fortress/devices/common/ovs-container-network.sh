...
# IPs are assigned to FTS, not FTS bridge
setup_lan_gateway() {
    log_section "Setting Up FTS Bridge"

    # FTS bridge is Layer 2 only - NO IP
    # IP is on FTS (LAN) - access control via OpenFlow fingerprint policies
    log_info "FTS bridge is Layer 2 only (no IP)"
    log_info "IP will be assigned to FTS"
...
NOTE: Containers use podman-compose built-in networking (containers_fts-internal).
      OVS provides VLAN interface (FTS) for LAN traffic only.

...
# ============================================================
# HOSTAPD-OVS BUILD (REQUIRED)
# ============================================================
# Build hostapd with OVS bridge support for direct WiFi→OVS integration.
# This is REQUIRED for Fortress - there is no fallback mode.
#
# Architecture: WiFi → OVS (FTS) directly → OpenFlow policy
# No intermediate Linux bridge - best performance
#
# The patch is in shared/hostapd-ovs/build-hostapd-ovs.sh
# ============================================================
build_hostapd_ovs() {
    log_info "Building hostapd with OVS bridge support (REQUIRED)..."
...
ExecStartPre=-/usr/bin/ovs-vsctl --may-exist add-port FTS ${WIFI_24GHZ_STABLE}
ExecStart=${hostapd_bin} -B -P /run/hostapd-24ghz.pid /etc/hostapd/hostapd-24ghz.conf
ExecStop=-/bin/kill -TERM \$MAINPID
ExecStopPost=-/usr/bin/ovs-vsctl --if-exists del-port FTS ${WIFI_24GHZ_STABLE}
...
ExecStartPre=-/usr/bin/ovs-vsctl --may-exist add-port FTS ${WIFI_5GHZ_STABLE}
ExecStart=${hostapd_bin} -B -P /run/hostapd-5ghz.pid /etc/hostapd/hostapd-5ghz.conf
ExecStop=-/bin/kill -TERM \$MAINPID
ExecStopPost=-/usr/bin/ovs-vsctl --if-exists del-port FTS ${WIFI_5GHZ_STABLE}

...
# Validation patterns for hostapd config values
_SSID_RE = re.compile(r'^[a-zA-Z0-9 _\-\.]{1,32}$')
_CHANNEL_RE = re.compile(r'^(auto|[0-9]{1,3})$')


def _sanitize_hostapd_value(value):
    """Reject values containing newlines or control chars (hostapd injection)."""
    if not value:
        return value
...
def _connect_with_wpa_supplicant(ssid, password, interface='wlan0'):
    ...
    # Build wpa_supplicant config safely
    # Sanitize SSID: reject control characters
    safe_ssid = ssid.replace('"', '').replace('\\', '')
    if any(ord(c) < 32 for c in safe_ssid):
        return False, 'SSID contains invalid characters', None
...
'''
        ...
        # Use wpa_passphrase to generate a hex PSK (avoids escaping issues)
        psk_output, psk_ok = run_command(['wpa_passphrase', ssid, password], timeout=5)
        if psk_ok and psk_output:
            # Extract the hex PSK line (not the commented plaintext line)
            psk_hex = None
            for psk_line in psk_output.split('\n'):
                psk_line = psk_line.strip()
                if psk_line.startswith('psk=') and not psk_line.startswith('#'):
                    psk_hex = psk_line.split('=', 1)[1]
                    break

            if psk_hex and len(psk_hex) == 64:
                config = f'''ctrl_interface=DIR=/var/run/wpa_supplicant GROUP=netdev
update_config=1
country={country}
...
'''
                ...
                return False, 'Failed to generate PSK hash', None
        else:
            return False, 'wpa_passphrase not available', None
...
def api_hotspot():
    ...
    if not request.environ.get('_auth_checked'):
        # Inline auth check for mixed GET/POST handler
    ...
    # Validate SSID (no newlines, control chars, reasonable length)
    if not _SSID_RE.match(ssid):
        return jsonify({'success': False, 'error': 'Invalid SSID (alphanumeric, 1-32 chars)'}), 400
    ...
    if password and len(password) > 63:
        return jsonify({'success': False, 'error': 'Password must be at most 63 characters'}), 400
    ...
    try:
        ...
        # Build config line by line (no f-string interpolation of user values)
        lines = [
            '# HookProbe Guardian - Hotspot Configuration',
            ...
            'wpa=2',
            f'wpa_passphrase={password if password else "changeme123"}',

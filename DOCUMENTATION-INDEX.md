# HookProbe Documentation Index

> **This file redirects to the main documentation index.**

For complete documentation, see: **[docs/DOCUMENTATION-INDEX.md](docs/DOCUMENTATION-INDEX.md)**

---

## Quick Links

- **[README.md](README.md)** — Project overview and features
- **[QUICK-START.md](QUICK-START.md)** — 3-step installation guide
- **[ARCHITECTURE.md](ARCHITECTURE.md)** — Repository structure

## Product Documentation

| Product | Description | Link |
|---------|-------------|------|
| **Guardian** | Portable travel hotspot | [products/guardian/README.md](products/guardian/README.md) |
| **Fortress** | Full SDN with VLAN segmentation | [products/fortress/README.md](products/fortress/README.md) |
| **Nexus** | ML/AI compute hub | [products/nexus/README.md](products/nexus/README.md) |
| **Sentinel** | Lightweight edge validator | [products/sentinel/README.md](products/sentinel/README.md) |
| **MSSP** | Cloud federation & VPN | [products/mssp/web/README.md](products/mssp/web/README.md) |

## Full Documentation

→ **[docs/DOCUMENTATION-INDEX.md](docs/DOCUMENTATION-INDEX.md)** — Complete navigation guide

---

**Version**: 5.0.0 Cortex
**Last Updated**: 2025-12-07
